...
{
  int bit;
  DELAY_US(50); // recovery, min 1us
  onewire_line_low(ow);
  DELAY_US(5); // hold min 1us
  onewire_line_release(ow);
  DELAY_US(30); // 15us window
...
for (i = 0; i < 65; i++) __delay_cycles(100000);
  ...
  if ((scratchpad[1]&128) == 0){
	...
	tmp = (scratchpad[1]<<8)|scratchpad[0];
  ...
  temperature = (scratchpad[0]&15);			//выделяем с помощью битовой маски дробную часть
  temperature = (temperature<<1) + (temperature<<3);	//преобразуем в целое число
