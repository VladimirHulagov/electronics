...
for (i = 0; i < 65; i++) __delay_cycles(100000);
  ...
  scratchpad[0] = onewire_read_byte(&ow);  //LSB
  scratchpad[1] = onewire_read_byte(&ow);  //MSB
  //выводим значение целой части показателя измеренной температуры
  temperature[0] = (scratchpad[0]>>4)|((scratchpad[1]&7)<<4);
  printf("+%i",temperature[0]);
  
  //выводим значение дробной части показателя измеренной температуры
  temperature[1] = (scratchpad[0]&15);			//выделяем с помощью битовой маски дробную часть
  temperature[1] = (temperature[1]<<1) + (temperature[1]<<3);	//преобразуем в целое число
  temperature[1] = (temperature[1]>>4);			//делим на 16 или умножаем на 0.0625
  printf(".%i\n",temperature[1]); 

  if (temperature[0] < 29)
  {
  	printf("temperature <29; power on\n");
  	P1OUT |= BIT6;
  }
  else 
  {
	printf("temperature >29; power off\n");
	P1OUT &=~BIT6;
  }
...
int main()
{
  ...
  P1DIR = BIT0 + BIT6;                             // All P1.x outputs
